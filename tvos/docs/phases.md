...
- ~~**1080p60 HDR default profile.**~~ ✅ implemented 2026-05-07. `AppSettings` defaults: `.res1080p` / `.fps60` / `.h265hdr` / `30 000 kbps` ([`AppSettings.swift`](../ChiakiTV/Models/AppSettings.swift)).
  - Initially shipped as `.res2160p` on 2026-05-06; corrected on 2026-05-07 after [run7](reports/2026-05-07-01-30-run7.md): **PS5 Remote Play caps at 1080p**. Requesting 2160p triggers `Nagare did not init! AvCap failed to initialize video: [InitResult:-6]` and the PS5 disconnects.
  - `video_profile_auto_downgrade=true` does not catch this — the downgrade branches in [`lib/src/ctrl.c:1442-1461`](../../lib/src/ctrl.c) only fire for PS4 server types.
  - Bitrate stays at 30 000 kbps because `bwKbpsSent` ([`lib/src/launchspec.c:24`](../../lib/src/launchspec.c)) is a *cap*, not a target — the PS5 only uses what it needs, and 1080p60 HDR genuinely benefits from the headroom over upstream's 15 Mbps SDR preset. See [`decisions.md`](decisions.md#ps5-remote-play-caps-at-1080p--defaults-target-1080p60-hdr-not-4k60-hdr).
  - HDR pipeline (HDR10 colorimetry on `CMFormatDescription`, BT.2020 PQ Metal render path, `AVDisplayCriteria(refreshRate:formatDescription:)`) all stays — 1080p60 HDR is exactly what it was built to render.
